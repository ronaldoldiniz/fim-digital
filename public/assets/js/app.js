 ...
 */
function verificarVibracao(input, silent = false) {
    ...
    if (valor <= 3.2) {
        ...
        if (!silent) confirmarVibracao(input, valor);
    } else {
...
function avancarStatus(registroId) {
    const formBancada = document.getElementById('formBancada');
    const formCliente = document.getElementById('formCliente');

    // Função interna para disparar o avanço real
    const processarAvanco = function() {
        salvarAjax('../actions/alterar_status.php', { registro_id: registroId, acao: 'avancar' }, function(data) {
            if (data.novo_status) {
    ...
    // Sobrescrever a função callback do salvarAjax internamente para detectar erros de validação
    const handleAvanco = function(url, formData) {
        fetch(url, { method: 'POST', body: formData })
        .then(res => res.json())
        .then(data => {
            if (data.sucesso) {
                // Tenta avançar status após salvar com sucesso
                fetch('../actions/alterar_status.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: `registro_id=${registroId}&acao=avancar`
                })
                .then(r => r.json())
                .then(d => {
                    if (d.sucesso) {
                        mostrarToast(`Status atualizado para: ${formatarStatus(d.novo_status)}`, 'sucesso');
                        setTimeout(() => location.reload(), 1000);
                    } else {
                        alert("Não foi possível avançar:\n\n" + d.mensagem);
                        mostrarToast(d.mensagem, 'erro');
                    }
                });
            } else {
                mostrarToast('Erro ao salvar formulário.', 'erro');
            }
        });
    };

    if (formBancada) {
        handleAvanco('../actions/salvar_bancada.php', new FormData(formBancada));
    } else if (formCliente) {
        handleAvanco('../actions/salvar_cliente.php', new FormData(formCliente));
    } else {
        fetch('../actions/alterar_status.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
...
/**
 * Retorna o status da FIM (usando modal Bootstrap em vez de confirm() nativo)
 */
function voltarStatus(registroId) {
    const modalId = 'modalVoltarStatus';
    let modal = document.getElementById(modalId);
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header bg-warning text-dark">
                        <h5 class="modal-title"><i class="bi bi-arrow-left-circle me-2"></i> Voltar Status</h5>
                    </div>
                    <div class="modal-body">
                        <p class="fs-5">Deseja realmente <strong>RETORNAR</strong> esta FIM para a etapa anterior?</p>
                        <p class="text-muted mb-0"><i class="bi bi-info-circle me-1"></i> Esta ação irá reverter o status atual para o estágio anterior do processo.</p>
                    </div>
                    <div class="modal-footer justify-content-center">
                        <button type="button" class="btn btn-secondary btn-industrial" id="btnVoltarCancelar">
                            <i class="bi bi-x-circle"></i> Cancelar
                        </button>
                        <button type="button" class="btn btn-warning btn-industrial text-dark" id="btnVoltarConfirmar">
                            <i class="bi bi-arrow-left-circle"></i> Sim, Voltar Status
                        </button>
                    </div>
                </div>
            </div>
    ...
    document.getElementById('btnVoltarCancelar').onclick = () => bsModal.hide();

    document.getElementById('btnVoltarConfirmar').onclick = () => {
        bsModal.hide();
        const btnConfirmar = document.getElementById('btnVoltarConfirmar');
        if (btnConfirmar) {
            btnConfirmar.disabled = true;
            btnConfirmar.innerHTML = '<i class="bi bi-hourglass-split"></i> Aguarde...';
        }

        fetch('../actions/alterar_status.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: `registro_id=${registroId}&acao=voltar`
        })
        .then(r => r.json())
        .then(data => {
            if (data.sucesso) {
                mostrarToast(`✅ Status retornado para: ${formatarStatus(data.novo_status)}`, 'sucesso');
                setTimeout(() => location.reload(), 1500);
            } else {
                mostrarToast('❌ ' + (data.mensagem || 'Erro ao voltar status.'), 'erro');
            }
        })
        .catch(() => {
            mostrarToast('❌ Erro de comunicação com o servidor.', 'erro');
